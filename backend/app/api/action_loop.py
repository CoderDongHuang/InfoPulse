...
async def detail(aid:str,ctx:TenantContext=Depends(get_tenant_context),db:AsyncSession=Depends(get_db)):
    require_permission(ctx,"action.read"); a=await get_action(db,ctx,aid); steps=(await db.scalars(select(ActionStep).where(ActionStep.action_id==aid).order_by(ActionStep.sequence))).all(); runs=(await db.scalars(select(ActionRun).where(ActionRun.action_id==aid).order_by(ActionRun.started_at.desc()))).all(); receipts=(await db.scalars(select(ActionReceipt).where(ActionReceipt.action_id==aid).order_by(ActionReceipt.received_at.desc()))).all()
    return {**serialize(a),"steps":[{"id":s.id,"sequence":s.sequence,"channel":s.channel,"tool_key":s.tool_key,"status":s.status,"requires_approval":s.requires_approval,"error_message":s.error_message} for s in steps],"runs":[{"id":r.id,"idempotency_key":r.idempotency_key,"status":r.status,"attempt":r.attempt,"output":r.output,"error_message":r.error_message,"started_at":r.started_at,"finished_at":r.finished_at} for r in runs],"receipts":[{"id":r.id,"run_id":r.run_id,"step_id":r.step_id,"channel":r.channel,"external_reference":r.external_reference,"response_code":r.response_code,"receipt_payload":r.receipt_payload,"evidence_content_ids":r.evidence_content_ids,"received_at":r.received_at} for r in receipts]}
