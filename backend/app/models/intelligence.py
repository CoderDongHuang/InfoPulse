...
class AlertRule(Base):
    ...
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uuid_string);user_id:Mapped[str]=mapped_column(ForeignKey("users.id",ondelete="CASCADE"),index=True);name:Mapped[str]=mapped_column(String(200));rule_type:Mapped[str]=mapped_column(String(30),index=True);config:Mapped[dict]=mapped_column(JSON,default=dict);combinator:Mapped[str]=mapped_column(String(10),default="all");severity:Mapped[str]=mapped_column(String(20),default="warning");enabled:Mapped[bool]=mapped_column(Boolean,default=True);assignee_id:Mapped[str|None]=mapped_column(ForeignKey("users.id",ondelete="SET NULL"),index=True);created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utc_now);updated_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utc_now,onupdate=utc_now)
class AlertIncident(Base):
    __tablename__="alert_incidents";__table_args__=(UniqueConstraint("rule_id","event_id","fingerprint",name="uq_alert_incident_fingerprint"),)
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uuid_string);rule_id:Mapped[str]=mapped_column(ForeignKey("alert_rules.id",ondelete="CASCADE"),index=True);user_id:Mapped[str]=mapped_column(ForeignKey("users.id",ondelete="CASCADE"),index=True);event_id:Mapped[str|None]=mapped_column(ForeignKey("events.id",ondelete="SET NULL"),index=True);status:Mapped[str]=mapped_column(String(20),default="triggered",index=True);severity:Mapped[str]=mapped_column(String(20));title:Mapped[str]=mapped_column(String(300));reason:Mapped[str]=mapped_column(Text);evidence:Mapped[list]=mapped_column(JSON,default=list);fingerprint:Mapped[str]=mapped_column(String(64));assignee_id:Mapped[str|None]=mapped_column(ForeignKey("users.id",ondelete="SET NULL"),index=True);is_false_positive:Mapped[bool]=mapped_column(Boolean,default=False);triggered_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utc_now,index=True);acknowledged_at:Mapped[datetime|None]=mapped_column(DateTime(timezone=True));closed_at:Mapped[datetime|None]=mapped_column(DateTime(timezone=True))
class AlertAction(Base):
    __tablename__="alert_actions"
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uuid_string);incident_id:Mapped[str]=mapped_column(ForeignKey("alert_incidents.id",ondelete="CASCADE"),index=True);user_id:Mapped[str]=mapped_column(ForeignKey("users.id",ondelete="CASCADE"),index=True);action:Mapped[str]=mapped_column(String(30));note:Mapped[str]=mapped_column(Text,default="");before_status:Mapped[str]=mapped_column(String(20));after_status:Mapped[str]=mapped_column(String(20));created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utc_now,index=True)
class AlertReplayRun(Base):
    __tablename__="alert_replay_runs"
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uuid_string);rule_id:Mapped[str]=mapped_column(ForeignKey("alert_rules.id",ondelete="CASCADE"),index=True);user_id:Mapped[str]=mapped_column(ForeignKey("users.id",ondelete="CASCADE"),index=True);from_at:Mapped[datetime]=mapped_column(DateTime(timezone=True));to_at:Mapped[datetime]=mapped_column(DateTime(timezone=True));matched_count:Mapped[int]=mapped_column(Integer,default=0);sample_results:Mapped[list]=mapped_column(JSON,default=list);created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=utc_now)
class BIQueryHistory(Base):
