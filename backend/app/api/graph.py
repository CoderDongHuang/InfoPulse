...
async def create_relation(event_id:str,p:RelationCreate,user:User=Depends(get_current_user),db=Depends(get_db)):
 await get_event_or_error(db,event_id);valid=set((await db.scalars(select(ContentItem.id).join(EventContent,EventContent.content_item_id==ContentItem.id).where(EventContent.event_id==event_id,ContentItem.id.in_(p.evidence_content_ids)))).all())
