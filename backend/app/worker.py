...
async def run(role: str):
    ...
    if role in {"media", "all"}: tasks.append(asyncio.create_task(media_worker_loop(stop)))
    try:
...
def main():
    ...
    parser.add_argument("--role", choices=["scheduler", "knowledge", "orchestration", "media", "all"], required=True)
