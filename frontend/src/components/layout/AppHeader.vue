...
const navGroups = [
  ...
  {
    label: "平台管理",
    items: [{ path: "/enterprise", label: "企业治理", icon: "Lock" }, { path: "/commercial-operations", label: "商业运营", icon: "DataAnalysis" }, { path: "/autonomy", label: "自治与合规", icon: "CircleCheck" }, { path: "/trust-network", label: "可信网络", icon: "Connection" }, { path: "/global-coordination", label: "全球协同", icon: "Share" }, { path: "/adaptive-os", label: "自适应 OS", icon: "Cpu" }, { path: "/provable-autonomy", label: "可证明自治", icon: "DocumentChecked" }, { path: "/planetary-resilience", label: "行星韧性", icon: "Aim" }, { path: "/cognitive-infrastructure", label: "认知基建", icon: "SetUp" }, { path: "/cognitive-commons", label: "认知公地", icon: "Connection" }, { path: "/developers", label: "开发者平台", icon: "Monitor" }, { path: "/sources", label: "数据源中心", icon: "Collection" }, { path: "/admin", label: "运行管理", icon: "Setting", adminOnly: true }],
  },
];
...
const active = (path: string) =>
  route.path === path ||
  (path === "/insight" && route.path === "/anti-scam") ||
  (path === "/events" && route.path.startsWith("/events/")) ||
...
<template>
  <aside class="topbar">
    ...
    <div class="account-area">
      ...
      >
        <el-icon><CollectionTag /></el-icon><span>{{ watchlist.count }}</span>
      </button>
      <template v-if="userStore.isLoggedIn">
        <el-dropdown trigger="click" @command="handleUserCommand">
          <button class="profile-button" type="button">
            <span class="avatar">{{
              userStore.userInfo?.username?.slice(0, 1).toUpperCase() || "U"
            }}</span>
            <span class="profile-copy"
              ><strong>{{ userStore.userInfo?.username }}</strong
              ><small>个人工作区</small></span
            >
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="history"
                ><el-icon><Clock /></el-icon>历史报告</el-dropdown-item
              >
