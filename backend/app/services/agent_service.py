...
async def gather(db,user_id,conversation,query,additions):
 ids=set();tools=[]
...
async def answer(query,rows):
 result={"answer":f"根据 {len(rows)} 条可访问来源，以下是与“{query}”相关的事实。","claims":[{"text":f"{x.title}：{(x.body or x.title)[:240]}","citation_indexes":[i]} for i,(x,_) in enumerate(rows[:5])]};model="evidence-rules-v1"
 if llm_is_configured():
  evidence=[{"index":i,"title":x.title,"text":x.body[:1000]} for i,(x,_) in enumerate(rows)];result=await complete_json("只依据证据回答JSON：answer和claims；claims含text、citation_indexes，每条事实必须引用。",f"问题:{query}\n证据:{evidence}",temperature=.1,max_tokens=2200);model=get_settings().LLM_MODEL
 valid=[]
